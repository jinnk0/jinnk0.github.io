...
#### 해결 방법
- 업종 및 매장 분위기 기반 태그 선택 -> AI를 통해 사용자의 취향, 매장 분위기를 고려하여 적절한 음악 추천
- 자영업자에게 간편하고 효율적인 저비용의 음악 큐레이션 제공
- 반복적인 의사결정의 최소화 + 고객 경험을 최적화하여 매출 증대 효과 노림

#### 핵심 기능 요약
- 스포티파이 연동 로그인 지원
- 기존 플레이리스트 정보 수집 (스포티파이 플레이리스트 또는 플레이리스트 캡처 이미지 활용)
- 매장 분위기 태그와 사용자 선호(기존 플레이리스트 정보를 통해 파악)를 반영한 AI 기반 음악 추천
- 추천 음악 중 마음에 드는 음악을 선택하여 개인 맞춤 플레이리스트 생성 가능
- 선택한 음악 데이터를 다음 추천에 반영하여 개인화 강화
...
- **기술 선택 이유**:  
	- FastAPI : 동기 처리에 최적화되어 있어 외부 API 연동과 모델 추론 시 빠른 응답 속도 제공
	- PostgreSQL : 관계형 데이터와 함께 임베딩·피처 벡터 저장이 가능한 범용성
	- MongoDB : 유연한 구조로 사용자 로그 저장에 적합
	- Redis : 느린 DB 쿼리 대신 반복 요청 데이터를 캐싱하여 응답 시간 단축
	- GCP : 무료 크레딧을 통한 비용 절감, 쉬운 사용법으로 단기간에 빠른 배포 가능
	- Airflow : 정기적 재학습 파이프라인 자동화에 용이
	- PyTorch + LightGCN : 사용자–음원 관계 모델링에서 성능 우수, 추론 속도도 양호
